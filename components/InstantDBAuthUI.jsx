'use client'
import db from '@/lib/instantdb'
import { useEffect } from 'react'
import { useRouter } from 'next/navigation'

export default function AuthUI() {
  const router = useRouter()
  const { user, isLoading } = db.useAuth()

  // If user is already logged in, redirect to home
  useEffect(() => {
    if (user && !isLoading) {
      router.replace('/')
    }
  }, [user, isLoading, router])

  const handleGoogleSignIn = () => {
    try {
      const baseURL = typeof window !== 'undefined' ? `${window.location.protocol}//${window.location.host}` : 'http://localhost:3000'
      // Redirect back to this page so we can handle the OAuth callback
      const redirectTo = `${baseURL}/auth`
      const url = db.auth.createAuthorizationURL({
        clientName: "google-button-for-web",
        redirectURL: redirectTo,
      })
      window.location.href = url
    } catch (err) {
      console.error('Error creating auth URL:', err)
    }
  }

  // Handle OAuth callback when provider redirects back with ?code
  useEffect(() => {
    const params = typeof window !== 'undefined' ? new URLSearchParams(window.location.search) : null
    if (!params) return
    const code = params.get('code')
    if (code) {
      (async () => {
        try {
          await db.auth.exchangeOAuthCode({ code })
          // cleanup url (remove query params) and redirect home
          router.replace('/')
        } catch (err) {
          console.error('Error exchanging OAuth code:', err)
        }
      })()
    }
  }, [router])

  return (
    <div className="min-h-screen flex items-center justify-center bg-gradient-to-br from-slate-50 to-slate-100">
      <div className="w-full max-w-md p-8 bg-white rounded-xl shadow-lg border border-slate-200">
        <h1 className="text-3xl font-bold text-slate-800 mb-2 text-center">
          <span className="text-orange-600">Crave</span>Store
        </h1>
        <p className="text-center text-slate-500 mb-8">
          Sign in to your account
        </p>

        <button
          onClick={handleGoogleSignIn}
          className="w-full py-3 bg-white border border-slate-300 hover:bg-slate-50 text-slate-700 font-medium rounded-lg transition flex items-center justify-center gap-3"
        >
          <svg className="w-5 h-5" viewBox="0 0 24 24">
            <path fill="currentColor" d="M22.56 12.25c0-.78-.07-1.53-.2-2.25H12v4.26h5.92c-.26 1.37-1.04 2.53-2.21 3.31v2.77h3.57c2.08-1.92 3.28-4.74 3.28-8.09z"/>
            <path fill="currentColor" d="M12 23c2.97 0 5.46-.98 7.28-2.66l-3.57-2.77c-.98.66-2.23 1.06-3.71 1.06-2.86 0-5.29-1.93-6.16-4.53H2.18v2.84C3.99 20.53 7.7 23 12 23z"/>
            <path fill="currentColor" d="M5.84 14.09c-.22-.66-.35-1.36-.35-2.09s.13-1.43.35-2.09V7.07H2.18C1.43 8.55 1 10.22 1 12s.43 3.45 1.18 4.93l2.85-2.22.81-.62z"/>
            <path fill="currentColor" d="M12 5.38c1.62 0 3.06.56 4.21 1.64l3.15-3.15C17.45 2.09 14.97 1 12 1 7.7 1 3.99 3.47 2.18 7.07l3.66 2.84c.87-2.6 3.3-4.53 6.16-4.53z"/>
          </svg>
          Continue with Google
        </button>

        <p className="text-xs text-slate-500 text-center mt-6">
          Sign in securely with your Google account
        </p>
      </div>
    </div>
  )
}
