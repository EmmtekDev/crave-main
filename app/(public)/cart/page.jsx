...
export default function Cart() {
    ...
    const { products, isLoading: productsLoading } = useProducts(); // Fetch from Instant DB
    ...
    const createCartArray = () => {
        setTotalPrice(0);
        const cartArray = [];
        // Try Instant DB products first, fall back to dummy products
                        ...
                        <tbody>
                            {
                                cartArray.map((item, index) => (
                                    <tr key={index} className="space-x-2">
                                        ...
                                        <td className="text-center">{currency}{(((item?.price || 0) / 100) * (item?.quantity || 0)).toLocaleString()}</td>
