...
export const useDeliveryByTracking = (trackingNumber) => {
  const { isLoading, error, data } = db.useQuery({ deliveries: {} })
  const delivery = trackingNumber
    ? (data?.deliveries || []).find(d => d.trackingNumber === trackingNumber) || null
    : null
  return { delivery, isLoading, error }
}

export const useUserDeliveries = (userId) => {
  const { isLoading, error, data } = db.useQuery({ deliveries: {} })
  const deliveries = userId
    ? (data?.deliveries || []).filter(d => d.userId === userId)
    : []
  return {
    deliveries,
    isLoading,
